...
function createPrizeCard(prizes, setWin, setPrize){
    let cardNumbers = [];
    for (let i = 0; i < 3; i++) {
        cardNumbers.push(prizes[Math.floor(Math.random() * prizes.length)])
    }
    if(cardNumbers[0] === cardNumbers[1] === cardNumbers[2]){
        setWin(true);
        setPrize(cardNumbers[0])
    }
    else{
        setWin(false);
        setPrize(0)
...
const ScratchableCard = ({setWin, setPrize}) => {
    prizeCard = createPrizeCard(prizes, setWin, setPrize);
